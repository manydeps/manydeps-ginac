...
// CHANGED size_t -> int !!!
template <typename T>
static int degree(const T& p) {
  return p.size() - 1;
}

template <typename T>
static typename T::value_type lcoeff(const T& p) {
  return p[p.size() - 1];
}
 ...
 *  @return            true if polynomial a was already unit normal, false
 * otherwise
 */
static bool normalize_in_field(umodpoly& a) {
  ...
  for (std::size_t k = a.size(); k-- != 0;) a[k] = a[k] * lc_1;
  return false;
}

/** Remove leading zero coefficients from polynomial.
 *
 *  @param[in, out] p     polynomial from which the zero leading coefficients
 * will be removed
 *  @param[in]      hint  assume all coefficients of order ≥ hint are zero
 */
template <typename T>
static void canonicalize(
    T& p, const typename T::size_type hint =
...
static typename enable_if<uvar_poly_p<T>::value, T>::type operator+(
    const T& a, const T& b) {
  int sa = a.size();
  int sb = b.size();
  if (sa >= sb) {
...
static void umodpoly_from_upoly(umodpoly& ump, const upoly& e,
...
static void umodpoly_from_ex(umodpoly& ump, const ex& e, const ex& x,
                             const cl_modint_ring& R) {
  // assert: e is in Z[x]
  int deg = e.degree(x);
  ump.resize(deg + 1);
...
static umodpoly umodpoly_to_umodpoly(const umodpoly& a, const cl_modint_ring& R,
                                     unsigned int m) {
  umodpoly e;
  if (a.empty()) return e;
 ...
 *  @param[out] q  polynomial quotient
 */
static void remdiv(const umodpoly& a, const umodpoly& b, umodpoly& r,
                   umodpoly& q) {
  int k, n;
  n = degree(b);
  k = degree(a) - n;
 ...
 *  @param[out] r  result
 */
static void expt_pos_Q(const umodpoly& w, const umodpoly& a, unsigned int q,
                       umodpoly& r) {
  if (w.empty()) return;
...
class modular_matrix {
 ...
 public:
  ...
  mvec::const_iterator row_begin(size_t row) const {
    return m.begin() + row * c;
  }
  mvec::const_iterator row_end(size_t row) const {
    return m.begin() + row * c + r;
  }
 ...
 */
static void distinct_degree_factor(const umodpoly& a_, vector<int>& degrees,
                                   upvec& ddfactors) {
  umodpoly a = a_;
 ...
 */
static void exteuclid(const umodpoly& a, const umodpoly& b, umodpoly& s,
                      umodpoly& t) {
 ...
 */
static void hensel_univar(const upoly& a_, unsigned int p, const umodpoly& u1_,
  ...
  cl_I maxmodulus =
      ash(calc_bound(a), maxdeg + 1);  // = 2 * calc_bound(a) * 2^maxdeg
  ...
  while (!e.empty() && modulus < maxmodulus) {
    ...
    umodpoly sigma = r;
    phi = umodpoly_to_upoly(tautilde) +
          umodpoly_to_upoly(q) * umodpoly_to_upoly(u1);
    umodpoly tau;
 ...
 *  @param[in]     x      symbol
 *  @param[in,out] prime  prime number to start trying modular factorization
 * with, output value is the prime number actually used
 */
static ex factor_univariate(const ex& poly, const ex& x, unsigned int& prime) {
  ...
  while (tocheck.size()) {
    ...
    while (true) {
      // call Hensel lifting
      hensel_univar(tocheck.top().poly, prime, part.left(), part.right(), f1,
                    f2);
      if (!f1.empty()) {
...
// forward declaration
static vector<ex> multivar_diophant(const vector<ex>& a_, const ex& x,
                                    const ex& c, const vector<EvalPoint>& I,
 ...
 */
static upvec multiterm_eea_lift(const upvec& a, const ex& x, unsigned int p,
                                unsigned int k) {
  ...
  for (size_t j = 1; j < r; ++j) {
    ...
    vector<ex> exsigma =
        multivar_diophant(mdarg, x, umodpoly_to_ex(beta, x), empty, 0, p, k);
    umodpoly sigma1;
 ...
 */
static void eea_lift(const umodpoly& a, const umodpoly& b, const ex& x,
                     unsigned int p, unsigned int k, umodpoly& s_,
                     umodpoly& t_) {
 ...
 *  @return   vector of polynomials (s_i)
 */
static upvec univar_diophant(const upvec& a, const ex& x, unsigned int m,
...
static ex hensel_multivar(const ex& a, const ex& x, const vector<EvalPoint>& I,
                          unsigned int p, const cl_I& l, const upvec& u,
                          const vector<ex>& lcU) {
  const size_t nu = I.size() + 1;
  ...
  for (size_t j = 2; j <= nu; ++j) {
    ...
    for (size_t k = 1; k <= deg; ++k) {
      if (!e.is_zero()) {
        ...
        if (!c.is_zero()) {
          vector<ex> deltaU =
              multivar_diophant(U1, x, c, newI, maxdeg, p, cl_I_to_uint(l));
          for (size_t i = 0; i < n; ++i) {
            deltaU[i] *= monomial;
            U[i] += deltaU[i];
...
/** Takes a factorized expression and puts the factors in a vector. The
 * exponents of the factors are discarded, e.g. 7*x^2*(y+1)^4 --> {7,x,y+1}. The
 * first element of the result is always the numeric coefficient.
 */
 ...
 *  @param[in]     vn       leading coefficient of u in x (x==first symbol in
 * syms)
 *  @param[in]     x        first symbol that appears in u
 *  @param[in]     syms_wox remaining symbols that appear in u
 *  @param[in]     f        vector containing the factors of the leading
 * coefficient vn
 *  @param[in,out] modulus  integer modulus for random number generation (i.e.
 * |a_i| < modulus)
 *  @param[out]    u0       returns the evaluated (univariate) polynomial
 *  @param[out]    a        returns the valid evaluation points. must have
 * initial size equal number of symbols-1 before calling generate_set
 */
static void generate_set(const ex& u, const ex& vn, const ex& x,
                         const exset& syms_wox, const exvector& f,
  ...
  while (true) {
    ...
    if (!is_a<numeric>(vn)) {
      // ... and for which the evaluated factors have each an unique prime
      // factor
      exvector fnum = f;
...
struct factorization_ctx {
  ...
  ex vn;
  exvector vnlst;   // leading coeff, factors of leading coeff
  ...
  ex try_next_evaluation_homomorphism() {
    ...
    cl_I B =
        ash(calc_bound(u, x), maxdeg + 1);  // = 2 * calc_bound(u,x) * 2^maxdeg
    cl_I l = 1;
    cl_I pl = prime;
    while (pl < B) {
      l = l + 1;
...
static ex factor_multivariate(const ex& poly, const exset& syms) {
  ...
  for (auto x : syms) {
    ...
    copy_if(syms.begin(), syms.end(), inserter(syms_wox, syms_wox.end()),
            [x](const ex& y) { return y != x; });
    ...
    if (!is_a<numeric>(ctx.cont)) {
      // content is a polynomial in one or more of remaining syms, let's start
      // over
...
static ex factor1(const ex& poly, unsigned options) {
  ...
  ex res = 1;
  factor_iter(sfpoly, [&](const ex& f, const ex& k) {
    if (is_a<add>(f)) {
...
ex factor(const ex& poly, unsigned options) {
  ex result = 1;
  factor_iter(poly, [&](const ex& f1, const ex& k1) {
    factor_iter(factor1(f1, options), [&](const ex& f2, const ex& k2) {
      result *= pow(f2, k1 * k2);
    });
  });
  return result;
